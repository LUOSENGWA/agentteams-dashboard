...
//
// 数据面（BFF 双平面）：
//   L1 = Controller Docker 代理（archive 直读 agent.json / exec 容器内 running-config
//        整对象写——live 热加载+落盘+MinIO 同步；旧 Controller 即开即用）
//   L2 = Controller #1216 REST（本团队 Worker 可读写；OFF→403；team leader 只读）
//   404（Worker 不存在/两平面均不可用）→ 整节隐藏；读 403 → 琥珀 L2 提示不报错。
...
export function WorkerApprovalControl({ workerName }: { workerName: string }) {
  const { off, l2Hint, loading, level, saving, error, setLevel, reload } = useWorkerApproval(workerName);
  ...
  return (
    <div className="rounded-lg border p-2.5 pt-2.5">
      ...
      {loading ? (
        ...
        <p className="py-2 text-[11.5px] text-amber-600">
          L2 账号无权限读取（需 L1 管理员凭据；Controller 升级含 #1216 审批端点后自动开放）
        </p>
      ) : level ? (
        <>
