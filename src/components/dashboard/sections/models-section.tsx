...
import { AlertTriangle, ArrowLeftRight, ArrowRight, ChevronDown, ChevronRight, Clock, Gauge, Globe, Info, Key, Link2, Loader2, Monitor, Pencil, Play, Plus, Route, Save, Server, ToggleLeft, ToggleRight, Trash2, Users, Zap } from 'lucide-react';
...
function routeToForm(route: AiRoute): RouteForm {
  return {
    ...
    authConfig: route.authConfig
      ? {
          enabled: Boolean(route.authConfig.enabled),
          allowedCredentialTypes: Array.isArray(route.authConfig.allowedCredentialTypes)
            ? [...route.authConfig.allowedCredentialTypes]
            : [],
          ...(Array.isArray(route.authConfig.allowedConsumers) && route.authConfig.allowedConsumers.length > 0
...
function MappingEditor({ value, onChange, label }: { value: ModelMappingRule[]; onChange: (_value: ModelMappingRule[]) => void; label?: string }) {
  return <div className="space-y-2">
    <div className="flex items-center justify-between">
      <div>
        <Label>{label ?? '模型映射'}</Label>
        <p className="text-[11px] text-muted-foreground mt-0.5">左边填请求模型别名（Worker 中选择的名称），右边填供应商实际模型名</p>
      </div>
      <Button type="button" variant="outline" size="sm" onClick={() => onChange([...value, { pattern: '', targetModel: '' }])}><Plus className="mr-1 size-3" />添加映射</Button>
    </div>
    {value.length > 0 && (
      <div className="hidden grid-cols-[1fr_auto_1fr] gap-2 text-xs text-muted-foreground px-1 md:grid">
        <span>请求模型别名</span>
        <span className="w-6" />
        <span>供应商实际模型</span>
      </div>
    )}
    {value.map((mapping, index) => <div className="flex items-center gap-2" key={index}>
      <Input aria-label="请求模型别名" value={mapping.pattern} placeholder="例: deepseek-chat" className="flex-1" onChange={(event) => onChange(value.map((item, itemIndex) => itemIndex === index ? { ...item, pattern: event.target.value } : item))} />
      <ArrowRight className="size-4 text-muted-foreground shrink-0" />
      <Input aria-label="供应商实际模型" value={mapping.targetModel} placeholder="例: deepseek-chat" className="flex-1" onChange={(event) => onChange(value.map((item, itemIndex) => itemIndex === index ? { ...item, targetModel: event.target.value } : item))} />
...
function ProviderDialog({ open, provider, onOpenChange }: { open: boolean; provider: LlmProviderResponse | null; onOpenChange: (_open: boolean) => void }) {
  ...
  // provider is already referenced by a route or when editing, or when no model
  // mappings are configured (empty-predicate routes would match all aliases and
  // cause binding table noise).
  const autoWireProviderRoute = (providerName: string) => {
    ...
    if (form.modelMappings.length === 0) {
      toast.info(`提供商 ${providerName} 已创建。请手动创建 AI 路由并配置模型映射，或添加模型映射后重试。`);
      return;
    }
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto"><DialogHeader><DialogTitle>{editing ? `编辑提供商 - ${provider?.name}` : '创建提供商'}</DialogTitle><DialogDescription>配置 AI 模型供应商，填入 API Key 后即可在路由中引用</DialogDescription></DialogHeader><div className="space-y-4 py-2">
    <div className="rounded-lg border border-blue-500/20 bg-blue-500/5 p-3 text-xs text-muted-foreground">
      <p><strong className="text-foreground">第 1 步：</strong>填写供应商名称、类型和 API Key。</p>
      <p className="mt-1"><strong className="text-foreground">第 2 步：</strong>如果使用非官方地址，填入自定义 Base URL（必须以 <code className="font-mono text-[11px]">/v1</code> 或 <code className="font-mono text-[11px]">/v3</code> 结尾）。</p>
      <p className="mt-1"><strong className="text-foreground">第 3 步：</strong>在「模型映射」中填写请求别名和对应的实际模型名。保存后会自动创建路由。</p>
    </div>
    <div className="grid gap-3 md:grid-cols-4">
      <div><Label>名称 *</Label><Input disabled={editing || pending} value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="例如 my-provider" /></div>
    ...
    <div>
      <Label>{editing ? `新增 Token（当前 ${provider?.tokenCount ?? 0} 个，留空保留）` : 'API Key *（多个用逗号分隔）'}</Label>
      <div className="flex gap-2">
        <Input disabled={pending || testing} type="password" value={tokenInput} onChange={(event) => setTokenInput(event.target.value)} className="flex-1" placeholder="sk-... 或 ark-..." />
        <Button type="button" variant="outline" size="sm" disabled={testing || (!tokenInput && !editing)} onClick={handleTest}>{testing ? <Loader2 className="w-3.5 h-3.5 mr-1 animate-spin" /> : <Play className="w-3.5 h-3.5 mr-1" />}测试连通性</Button>
      </div>
      <p className="text-[11px] text-muted-foreground mt-1">输入 API Key 后点「测试连通性」验证是否可用。多个 Key 用英文逗号分隔可实现负载轮换。</p>
    </div>
    ...
    {PROVIDERS_NEED_BASE_URL.has(form.type) && <div>
      <Label>自定义 Base URL</Label>
      <Input disabled={pending} value={form.baseUrl ?? ''} onChange={(event) => setForm({ ...form, baseUrl: event.target.value })} placeholder="https://api.example.com/v1" />
      <p className="text-[11px] text-muted-foreground mt-1">不填则用官方地址。必须以 <code className="font-mono text-[11px]">/v1</code> 或 <code className="font-mono text-[11px]">/v3</code> 结尾，指向 OpenAI 兼容的 Chat Completions API。</p>
    </div>}
    ...
    <MappingEditor label="模型映射（请求别名 → 实际模型）" value={form.modelMappings} onChange={(modelMappings) => setForm({ ...form, modelMappings })} />
...
function RouteDialog({ open, route, providerNames, fallbackConfigWritable, onOpenChange }: { open: boolean; route: AiRoute | null; providerNames: string[]; fallbackConfigWritable: boolean; onOpenChange: (_open: boolean) => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto"><DialogHeader><DialogTitle>{editing ? `编辑路由 - ${route?.name}` : '创建 AI 路由'}</DialogTitle><DialogDescription>路由决定请求模型别名如何匹配到上游提供商。每个上游可配置独立的模型映射。</DialogDescription></DialogHeader><div className="space-y-4 py-2">
    <div className="grid gap-3 md:grid-cols-3"><div><Label>路由名称 *</Label><Input disabled={editing || pending} value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="例如 agentteams-ark" /></div><div><Label>路径匹配类型</Label><select disabled={pending} className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 text-sm" value="PRE"><option value="PRE">前缀（PRE）</option></select></div><div><Label>路径匹配 *</Label><Input disabled={pending} value={form.pathPredicate.matchValue} onChange={(event) => setForm({ ...form, pathPredicate: { ...form.pathPredicate, matchValue: event.target.value } })} /></div></div>
    <div className="space-y-3 rounded-md border p-3"><div className="flex items-center justify-between"><Label>上游提供商与权重</Label><Button type="button" variant="outline" size="sm" disabled={pending} onClick={() => setForm({ ...form, upstreams: [...form.upstreams, { provider: '', weight: 0, modelMappings: [] }] })}><Plus className="mr-1 size-3" />添加上游</Button></div><p className="text-[11px] text-muted-foreground -mt-1">选择提供商并设置流量权重。多上游时权重总和必须为 100。</p>{form.upstreams.map((upstream, index) => { const selectedByOthers = new Set(form.upstreams.filter((_, upstreamIndex) => upstreamIndex !== index).map((item) => item.provider).filter(Boolean)); return <div className="space-y-2 rounded border p-3" key={index}><div className="grid gap-2 md:grid-cols-[1fr_110px_36px]"><select aria-label="上游提供商" disabled={pending} className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 text-sm" value={upstream.provider} onChange={(event) => updateUpstream(index, { provider: event.target.value })}><option value="">选择提供商...</option>{providerNames.map((name) => <option key={name} value={name} disabled={selectedByOthers.has(name)}>{name}</option>)}</select><Input aria-label="上游权重" disabled={pending} type="number" min="0" max="100" value={upstream.weight} onChange={(event) => updateUpstream(index, { weight: Number(event.target.value) })} /><Button type="button" variant="ghost" size="icon" disabled={pending || form.upstreams.length === 1} aria-label="删除上游" onClick={() => setForm({ ...form, upstreams: form.upstreams.filter((_, upstreamIndex) => upstreamIndex !== index) })}><Trash2 className="size-4 text-destructive" /></Button></div><MappingEditor label="上游模型映射（仅对此上游生效，覆盖提供商级别映射）" value={upstream.modelMappings} onChange={(modelMappings) => updateUpstream(index, { modelMappings })} /></div>; })}</div>
    <div className="space-y-3 rounded-md border p-3">
      <div className="flex items-center justify-between">
        <Label>请求模型匹配</Label>
        <Button type="button" variant="outline" size="sm" disabled={pending} onClick={() => setForm({ ...form, modelPredicates: [...form.modelPredicates, { matchType: 'EXACT', matchValue: '' }] })}><Plus className="mr-1 size-3" />添加匹配</Button>
      </div>
      <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-2.5 text-[11px] text-muted-foreground space-y-1">
        <p><strong className="text-foreground">精确（EXACT）：</strong>只匹配指定的模型别名。例如填 <code className="font-mono text-[11px]">deepseek-chat</code> 则只匹配这个别名。</p>
        <p><strong className="text-foreground">前缀（PRE）：</strong>匹配以该值开头的所有别名。例如填 <code className="font-mono text-[11px]">kimi-</code>（不要加 *）则匹配 <code className="font-mono text-[11px]">kimi-k3</code> 等。</p>
        <p className="text-amber-600/80">不添加任何匹配规则 = 匹配所有请求模型别名（通配符路由），容易造成绑定表混乱，不建议使用。</p>
      </div>
      {form.modelPredicates.length === 0 && (
        <p className="text-xs text-muted-foreground italic">尚未添加匹配规则，当前为通配符路由（不推荐）</p>
      )}
      {form.modelPredicates.map((predicate, index) => <div className="grid gap-2 md:grid-cols-[150px_1fr_36px]" key={index}><select aria-label="模型匹配类型" disabled={pending} className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 text-sm" value={predicate.matchType} onChange={(event) => setForm({ ...form, modelPredicates: form.modelPredicates.map((item, itemIndex) => itemIndex === index ? { ...item, matchType: event.target.value } : item) })}><option value="EXACT">精确（EXACT）</option><option value="PRE">前缀（PRE）</option></select><Input aria-label="模型匹配值" disabled={pending} value={predicate.matchValue} placeholder={predicate.matchType === 'EXACT' ? '例: deepseek-chat' : '例: kimi-'} onChange={(event) => setForm({ ...form, modelPredicates: form.modelPredicates.map((item, itemIndex) => itemIndex === index ? { ...item, matchValue: event.target.value } : item) })} /><Button type="button" variant="ghost" size="icon" disabled={pending} aria-label="删除模型匹配" onClick={() => setForm({ ...form, modelPredicates: form.modelPredicates.filter((_, itemIndex) => itemIndex !== index) })}><Trash2 className="size-4 text-destructive" /></Button></div>)}</div>
...
export function ModelsSection() {
  ...
  return <div className="space-y-6">
    ...
    <div className="flex items-start gap-2 rounded-lg border border-cyan-500/20 bg-cyan-500/5 p-3 text-xs text-muted-foreground">
      <Info className="size-3.5 text-cyan-500 shrink-0 mt-0.5" />
      <span>所有 AI 路由共享网关路径前缀（默认 <code className="font-mono text-[11px]">/v1</code>），网关通过请求体中的 <code className="font-mono text-[11px]">model</code> 别名匹配对应路由分发到不同提供商；创建时给不同提供商配置不同的模型映射别名即可实现多路分发。</span>
    </div>
...
function RateLimitSection({ routes }: { routes: Array<{ name: string }> | undefined }) {
  ...
  return <div className="space-y-3">
    ...
    {configs.map((config, idx) => <div key={`${config.routeName}-${idx}`} className={`flex items-center gap-3 p-3 rounded-lg border ${config.enabled ? 'border-border bg-card/50' : 'border-border/50 bg-muted/20 opacity-60'}`}><button type="button" onClick={() => handleToggle(idx)} role="switch" aria-checked={config.enabled} aria-label={`限流规则 ${config.routeName} 启用状态`}>{config.enabled ? <ToggleRight className="size-5 text-emerald-500" aria-hidden="true" /> : <ToggleLeft className="size-5 text-gray-400" aria-hidden="true" />}</button><div className="flex-1 min-w-0"><div className="flex items-center gap-2"><Route className="size-3.5 text-muted-foreground" /><span className="text-xs font-medium">{config.routeName}</span>{config.perConsumer && <Badge variant="secondary" className="text-[11px]">按 Consumer</Badge>}</div><div className="flex items-center gap-3 mt-1 text-[10px] text-muted-foreground">{config.rps > 0 && <span className="flex items-center gap-1"><Zap className="size-3" />{config.rps} req/s</span>}{config.rpm > 0 && <span className="flex items-center gap-1"><Clock className="size-3" />{config.rpm} req/min</span>}{config.burst > 0 && <span>突发: {config.burst}</span>}</div></div><input type="number" className="w-16 h-7 rounded border border-input bg-transparent px-1.5 text-xs text-center" value={config.rps} onChange={(e) => handleUpdateField(idx, 'rps', parseInt(e.target.value) || 0)} title="RPS" /><span className="text-[10px] text-muted-foreground">req/s</span><Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => handleRemove(idx)}><Trash2 className="size-3.5 text-destructive" /></Button></div>)}
